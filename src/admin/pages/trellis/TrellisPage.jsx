...
export default function TrellisPage() {
  ...
  const fetchList = async (page = 1, pageSize = 20, search = '') => {
    try {
      ...
      const res = await referencesApi.trellis.list(params);
      setData(res.results || []);
      setPagination({ current: page, pageSize, total: res.count || 0 });
    } catch (e) { message.error('Не удалось загрузить шпалеры'); }
  ...
  const handleDelete = async (id) => {
    try { await referencesApi.trellis.delete(id); message.success('Удалено'); fetchList(pagination.current, pagination.pageSize, q); }
    catch { message.error('Ошибка удаления'); }
  };

  const exportXlsx = () => {
    const rows = (data || []).map(r => ({ id: r.id, type: r.type, count: r.count }));
    exportSimpleSheet(rows, [
      { header: 'ID', key: 'id', width: 10 },
      { header: 'Тип', key: 'type', width: 24 },
      { header: 'Кол-во', key: 'count', width: 14 },
    ], 'trellis.xlsx', 'Trellis');
  };

  return (
    <div>
      <PageHeader title="Шпалеры" extra={<Space><Button onClick={exportXlsx}>Экспорт</Button></Space>} />
      <TableLayout searchPlaceholder="Поиск по типу" onSearchChange={(val) => { setQ(val); fetchList(1, pagination.pageSize, val); }} onOpenFilters={() => setOpen(true)}>
        <Table
          columns={[
            ...
            { title: 'Кол-во', dataIndex: 'count', align: 'right', render: (v) => formatNumberShort(v) },
