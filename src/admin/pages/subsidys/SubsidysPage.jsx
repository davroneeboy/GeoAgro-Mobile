...
export default function SubsidysPage() {
  ...
  const fetchList = async (page = 1, pageSize = 20, search = '') => {
    try {
      ...
      const res = await referencesApi.subsidys.list(params);
      setData(res.results || []);
      setPagination({ current: page, pageSize, total: res.count || 0 });
    } catch (e) { message.error('Не удалось загрузить субсидии'); }
  ...
  const handleDelete = async (id) => {
    try { await referencesApi.subsidys.delete(id); message.success('Удалено'); fetchList(pagination.current, pagination.pageSize, q); }
    catch { message.error('Ошибка удаления'); }
  };

  const exportXlsx = () => {
    const rows = (data || []).map(r => ({ id: r.id, purpose: r.purpose, amount: r.amount }));
    exportSimpleSheet(rows, [
      { header: 'ID', key: 'id', width: 10 },
      { header: 'Назначение', key: 'purpose', width: 28 },
      { header: 'Сумма', key: 'amount', width: 16 },
    ], 'subsidys.xlsx', 'Subsidys');
  };

  return (
    <div>
      <PageHeader title="Субсидии" extra={<Space><Button onClick={exportXlsx}>Экспорт</Button></Space>} />
      <TableLayout searchPlaceholder="Поиск по назначению" onSearchChange={(val) => { setQ(val); fetchList(1, pagination.pageSize, val); }} onOpenFilters={() => setOpen(true)}>
        <Table
          columns={[
      ...
      <FilterDrawer open={open} onClose={() => setOpen(false)} onReset={() => setOpen(false)} onApply={() => setOpen(false)}>
        <div className="text-xs text-gray-500 mb-1">Фильтр по назначению</div>
