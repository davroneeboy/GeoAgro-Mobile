...
const HTTP_METHOD_COLORS = { GET: "blue", POST: "green", PUT: "orange", PATCH: "purple", DELETE: "red" };
const STATUS_COLORS = { 200: "green", 201: "green", 400: "orange", 401: "red", 403: "red", 404: "red", 500: "red" };
...
export default function MyLogsPage() {
    ...
    try {
      ...
      Object.entries(filters).forEach(([key, value]) => {
        if (value !== null && value !== "" && value !== undefined) {
          if ((key === "date_from" || key === "date_to") && value) params.append(key, value.toISOString());
          else params.append(key, value);
        }
      });
      params.append("limit", pagination.pageSize);
      params.append("offset", (pagination.current - 1) * pagination.pageSize);
      const resp = await fetch(`${API_BASE_URL1}api/logs/my/?${params}`, { headers: { Authorization: `Bearer ${authState.accessToken}` } });
  ...
  useEffect(() => { fetchMyLogs(); /* eslint-disable-next-line */ }, [filters, pagination.current, pagination.pageSize]);

  const handleFilterChange = (key, value) => { setFilters((prev) => ({ ...prev, [key]: value })); setPagination((p) => ({ ...p, current: 1 })); };
  const handleDateRangeChange = (dates) => { setFilters((prev) => ({ ...prev, date_from: dates?.[0] || null, date_to: dates?.[1] || null })); setPagination((p) => ({ ...p, current: 1 })); };

  const handleExport = async () => {
    try {
      message.loading({ content: "Eksport...", key: "exp" });
      const params = new URLSearchParams();
      Object.entries(filters).forEach(([key, value]) => {
        if (value !== null && value !== "" && value !== undefined) {
          if ((key === "date_from" || key === "date_to") && value) params.append(key, value.toISOString());
      ...
      const ws = XLSX.utils.json_to_sheet(rows, { header: ["ID","Vaqt","Metod","Yo'l","IP manzil","Status","Bajarilish vaqti (ms)","Muvaffaqiyatli","Xatolik","Xatolik xabari"] });
      ws["!cols"] = [{ width: 8 },{ width: 20 },{ width: 10 },{ width: 50 },{ width: 16 },{ width: 10 },{ width: 20 },{ width: 14 },{ width: 10 },{ width: 40 }];
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "My Logs");
      const buf = XLSX.write(wb, { bookType: "xlsx", type: "array" });
      saveAs(new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), `my_logs_${new Date().toISOString().split("T")[0]}.xlsx`);
      message.success({ content: "Eksport tayyor", key: "exp" });
    } catch (e) { message.error({ content: "Eksport xatosi: " + e.message, key: "exp" }); }
  };

  const textLight = { color: "#e5e7eb" };
  const columns = useMemo(() => [
    { title: <span style={textLight}>ID</span>, dataIndex: "id", key: "id", width: 80, render: (v) => <span style={textLight}>{v}</span> },
    { title: <span style={textLight}>Vaqt</span>, dataIndex: "timestamp", key: "timestamp", width: 180, render: (ts) => (<div className="flex items-center gap-2"><ClockCircleOutlined style={{ color: "#9ca3af" }} /><span style={textLight}>{ts ? new Date(ts).toLocaleString("ru-RU") : "—"}</span></div>) },
    { title: <span style={textLight}>Metod</span>, dataIndex: "method", key: "method", width: 90, render: (m) => <Tag color={HTTP_METHOD_COLORS[m] || "default"}>{m}</Tag> },
    { title: <span style={textLight}>Yo'l</span>, dataIndex: "path", key: "path", width: 300, render: (p) => (<div className="flex items-center gap-2"><GlobalOutlined style={{ color: "#9ca3af" }} /><span style={textLight} className="font-mono text-sm">{p || "—"}</span></div>) },
    { title: <span style={textLight}>IP</span>, dataIndex: "ip_address", key: "ip_address", width: 160, render: (ip) => <span style={textLight} className="font-mono text-sm">{ip || "—"}</span> },
    { title: <span style={textLight}>Status</span>, dataIndex: "response_status", key: "response_status", width: 100, render: (s) => <Tag color={STATUS_COLORS[s] || "default"}>{s}</Tag> },
    { title: <span style={textLight}>Vaqt (ms)</span>, dataIndex: "execution_time_ms", key: "execution_time_ms", width: 120, render: (ms) => <span className="font-mono" style={{ color: "#e5e7eb" }}>{Number(ms ?? 0)}</span> },
    { title: <span style={textLight}>Amallar</span>, key: "actions", width: 80, render: (_, rec) => (<Tooltip title="Tafsilotlar"><Button type="text" icon={<EyeOutlined />} onClick={() => { setSelectedLog(rec); setModalVisible(true); }} style={{ color: "#60a5fa" }} /></Tooltip>) },
  ], [textLight]);

  if (loading && logs.length === 0) return <Spin tip="Loglar yuklanmoqda..." size="large" className="w-full flex justify-center items-center min-h-screen" />;
          ...
          <Row gutter={[12, 12]}>
            <Col xs={24} md={6}>
              <div className="mb-2 sm:mb-4">
                ...
                <Select allowClear style={{ width: "100%" }} placeholder="Metod" value={filters.method} onChange={(v)=>handleFilterChange("method", v)} className="admin-logs-select">
            ...
            <Col xs={24} md={6}>
              <div className="mb-2 sm:mb-4">
                ...
                <Select allowClear style={{ width: "100%" }} placeholder="Status" value={filters.status} onChange={(v)=>handleFilterChange("status", v)} className="admin-logs-select">
