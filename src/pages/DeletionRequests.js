        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {requests.length === 0 ? (
            ...
            requests.map((request) => (
              <div 
                key={request.id} 
                className="bg-gray-800 rounded-lg p-4 border border-gray-700 cursor-pointer hover:bg-gray-750 hover:border-gray-600 transition-all"
                onClick={() => navigate(`/plantations/edit/${request.id}`, { 
                  state: { from: '/deletion-requests', page: page } 
                })}
              >
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    <p className="text-xs text-gray-400">Fermer</p>
                    <p className="text-sm font-semibold truncate">{request.farmer?.name || 'Ko\'rsatilmagan'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Viloyat</p>
                    <p className="text-sm font-semibold truncate">{regions[request.district?.region] || 'Ko\'rsatilmagan'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Maydon</p>
                    <p className="text-sm font-semibold">{request.total_area} GA</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Tuman</p>
                    <p className="text-sm font-semibold truncate">{request.district?.name || 'Ko\'rsatilmagan'}</p>
                  </div>
                </div>
