...
const LeftNav = () => {
  ...
  const toggleLeft = width - 12; // кнопка у правого края панели, не создаёт горизонтального скролла
                ...
                <Link to="/controllers" key={controller.id} className={`block ${collapsed ? 'px-2 py-2' : 'p-3'} border border-gray-600 rounded-lg bg-gray-700 hover:bg-gray-600 transition-colors`} title={collapsed ? (controller.first_name || controller.last_name ? `${controller.first_name} ${controller.last_name}` : controller.username) : undefined}>
                  {!collapsed ? (
                    <>
                      ...
                      <p className="text-xs text-gray-400">{(() => { const iso = (controller.last_login || "").replace(" ", "T"); const d = new Date(iso); return isNaN(d) ? "—" : d.toLocaleString("uz-UZ", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }); })()}</p>
